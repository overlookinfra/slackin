...
export default function slackin ({
  ...
  app.get('/', (req, res) => {
    ...
    let page = dom('html',
      ...
      splash({ coc, privacy, path, css, name, org, logo, channels, active, total, gcaptcha_sitekey})
    )
    res.type('html')
  ...
  app.post('/invite', json(), (req, res, next) => {
    ...
    if (coc && '1' != req.body.coc) {
      return res
      .status(400)
      .json({ msg: 'Must agree to the Code of Conduct' })
    }

    if (privacy && '1' != req.body.privacy) {
      return res
      .status(400)
      .json({ msg: 'Must agree to the Privacy Policy' })
    }
  ...
  app.get('/iframe/dialog', (req, res) => {
    ...
    let page = dom('html',
      dom("script src=https://www.google.com/recaptcha/api.js"),
      splash({ coc, privacy, path, name, org, channels, active, total, large, iframe: true, gcaptcha_sitekey })
    )
    res.type('html')
    res.send(page.toHTML())
  })
