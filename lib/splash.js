...
export default function splash ({ path, name, org, coc, privacy, logo, active, total, channels, large, iframe, gcaptcha_sitekey }){
  let div = dom('.splash',
    !iframe && dom('.logos',
      logo && dom('.logo.org'),
      dom('.logo.slack')
    ...
    dom('form id=invite',
      ...
      privacy && dom('.privacy',
        dom('label',
          dom('input type=checkbox name=privacy value=1'),
          'I agree to the ',
          dom('a', { href: privacy, target: '_blank' }, 'Privacy Policy'),
          '.'
        )
      ),
...
function style ({ logo, active, large, iframe } = {}){
  ...
  if (iframe) {
    ...
    css.add('.coc input, .privacy input', {
      position: 'relative',
      top: '-.2rem'
    })
  }

  css.add('.coc label, .privacy label', {
    cursor: 'pointer'
  })
  ...
  css.add('.coc input::after, .privacy input::after', {
    content: '""',
    display: 'inline-block',
    width: '1.5rem',
    height: '1.5rem',
  ...
  css.add('.coc input:checked::after, .privacy input:checked::after', {
    'background-position': 'right'
  })

  css.add('.coc a, privacy a', {
    color: '#666'
  })

  css.add('.coc a:hover, .privacy a:hover', {
  ...
  if (iframe) {
    css.add('.g-recaptcha', {
      'transform': large ? 'scale(1.0)' : 'scale(0.76)',
